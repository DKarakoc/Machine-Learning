 ...
 * 
 * @author + adaptations by Piotr Leszmann_s4771826,  Denis Karakoc_s4835093
 *
 */
	...
	public void paintComponent(Graphics g) {
		...
		for ( int i=0; i<mdp.getWidth(); i++ ) {
			for ( int j=0; j<mdp.getHeight(); j++ ) {				
                ...
                // this function uses getReward, which sets terminated to falls and disallows the agent to collect its reward in the program's logic
                // great example for the necessity of model-view separation
		//g2.drawString("Reward: \t\t"+mdp.getReward(), 30, screenHeight+25);
                g2.drawString("State Reward: \t\t"+mdp.getReward4Display(), 30, screenHeight+25);
		g2.drawString("#Actions: \t\t"+mdp.getActionsCounter(), 30, screenHeight+40);
                g2.drawString("Total Reward: \t\t"+String.format("%.2f",mdp.getTotalReward()), 30, screenHeight+55);
	}
